...
def read_csv_fast(path: Union[str, Path], nrows: Union[int, None] = None) -> pd.DataFrame:
    """Read CSV preferring the PyArrow engine when available, with safe fallback."""
    try:
        return pd.read_csv(path, nrows=nrows, engine="pyarrow")
    except Exception:
...
def load_tables(cfg: dict, logger: logging.Logger) -> dict:
    ...
    for p in folder.glob(csv_glob):
        try:
            ...
            df = read_csv_fast(p, nrows=sample_rows)
